...
- Re-pinned ellmer to GitHub HEAD
  `a64f94e644718c0598b01b0cd50a3c21c2646435`, including the refreshed
...
- Unified the MCP client, server, child-process guard, tests, and
  ...
  discovery/PKCE/token refresh support.
...
- Added the opt-in full-window `page_chat` layout with a 100%-width main
  ...
  explicitly retains its embedded width and disables unused native
  drawer/history presentation.
...
- Added opt-in support for ellmer provider-native `ContentCitation` /
  ...
  provider/model markup is never trusted directly.
...
- Hardened Data Shield input, tool, and output boundaries, fail-closed
  handling, sandbox/network policy, reviewer isolation, audit metadata,
  and protected-data schema/context behavior.

### Tools and customizations

- Tool results now expose a versioned, UI-neutral artifact v1 contract
  ...
  accessors. Streaming callbacks carry `artifact`, the optional official
  shinychat `display` adapter, and portable `value` fallback, so
  non-shinychat hosts can render native components without parsing
  shinychat HTML. Rich shinychat cards continue to use the official
  framed style. See
...
- Shiny Agents discovery now supports project and user
  ...
  `name` metadata.
- Updated CLI/session/tooling behavior and compatibility documentation
  for current ellmer, btw, shinychat, bslib, and Shiny development
  builds.
