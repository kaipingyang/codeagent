...
Improves on `recall_memories()` (which concatenates everything) by
asking a small/fast model to pick the memories relevant to the current
user query – Claude Code's sideQuery memory-selection pattern. Falls
back to the full concatenation when there is no query, the fast model is
unavailable, or selection fails, so behaviour never regresses.
