...
a non-NULL result makes the loop skip the tool
([`next`](https://rdrr.io/r/base/Control.html)). The async loop does
`coro::await(cb(request))` inside the same tryCatch, so a
