...
Use both: Path A is opt-in via
[`enable_btw_file_tools()`](https://github.com/kaipingyang/codeagent/reference/enable_btw_file_tools.md),
and coexists with the default tools. The LLM chooses the right tool for
each task: btw tools for project-local edits (safer, hash-verified),
default tools for absolute paths.
