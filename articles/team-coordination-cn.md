...
> **语言：**
> [English](https://kaipingyang.github.io/codeagent/articles/team-coordination.md)
> \| 简体中文

## 概述

codeagent
提供了多种不同的任务委派方式。应根据任务是否相互独立、是否受依赖关系约束、是在前台运行还是需要真正的非阻塞执行来选择：

- [`team_run()`](https://kaipingyang.github.io/codeagent/reference/team_run.md)
  是面向独立任务的固定扇出模式。工作进程并行运行，但函数会等待全部结果后才返回。
- [`team_coordinate()`](https://kaipingyang.github.io/codeagent/reference/team_coordinate.md)
  是由共享 SQLite 任务板支持的阻塞式工作窃取池，支持任务依赖
  DAG，也适合耗时不均的任务。
- [`team_lead()`](https://kaipingyang.github.io/codeagent/reference/team_lead.md)
  让一个 LLM 负责人分解目标、复核结果，并可在多个阻塞式
  [`team_coordinate()`](https://kaipingyang.github.io/codeagent/reference/team_coordinate.md)
  轮次之间重新规划。
- 前台 `Agent`
  工具用于委派一个子任务。在异步父轮次中，选择启用异步子智能体后，可以并发执行多个
  `Agent` 调用。
- `BackgroundAgent` 和 `/bg`
  是触发后立即返回的路径；结果会在后续轮次中显示。
...
工作进程数默认为
`min(length(tasks), parallelly::availableCores())`；如果 `parallelly`
不可用，则采用保守回退值。显式给出的 `n_workers`
也会被限制在同一个可感知 cgroup
的上限内。[`team_run()`](https://kaipingyang.github.io/codeagent/reference/team_run.md)
没有 worktree
...
`id`、`prompt`、`owner`、`status` 和 `result` 列的最终任务板。
...
设置 `worktree = TRUE` 可为每个工作进程请求一个临时的 detached Git
worktree：
...
`board_create(db_path = tempfile(fileext = ".sqlite"))` 创建
`tasks`、`deps` 和 `messages`
表，并以不可见形式返回数据库路径。[`board_claim()`](https://kaipingyang.github.io/codeagent/reference/board_claim.md)
可跨进程原子领取任务，[`board_complete()`](https://kaipingyang.github.io/codeagent/reference/board_complete.md)
记录结果，[`board_status()`](https://kaipingyang.github.io/codeagent/reference/board_status.md)
返回当前任务表。
...
`recipient = NULL` 表示广播。`board_messages(db, "coordinator")`
返回广播消息和发给该接收者的消息，`board_messages(db)`
则返回完整日志。这些消息不会自动插入工作进程的提示词：内置协调器会写入完成通知，但不会把消息表作为智能体之间的对话通道。

### 监视和展示任务板

可选的 `watcher` 包可用时，`board_watch(db, callback, latency = 0.3)`
会启动事件驱动的文件监视器；否则返回
`NULL`。`team_dashboard(db_path, poll_ms = 1500L)`
优先使用该监视器，并在不可用时回退到轮询。若要观察正在运行的
[`team_coordinate()`](https://kaipingyang.github.io/codeagent/reference/team_coordinate.md)，请传入固定的
...
智能体可以在聊天中调用前台 `Agent` 工具。常规路径是同步执行。客户端上的
worktree 隔离会强制使用 codeagent 自有的 Agent 实现，由它创建并清理临时
worktree：
...
`async_subagents` 允许多个前台 `Agent` 调用返回 promise
...
`background_agents = true` 会注册供模型调用的 `BackgroundAgent`
工具。它立即返回任务 id；内存注册表通过专用 `mirai` compute profile
轮询，并在后续 system reminder 中仅注入一次每个完成结果。`/bg <task>` 和
