...
### Architecture at a glance
                         ...
                         codeagent's central gate (sole authority, existing dual hooks)
     Host/user uploads data
         │ shield$register_data(df, name, sensitivity)
         ▼
     ┌────────────────────┐   builds a value index (deduped high-entropy values from
     │ Protected data      │   sensitive columns, LOCAL ONLY, never sent to the model)
     │ registration        │──────────────────────────────────────────┐
     │ bound to an envir / │                                          │
     │ auto-classified     │                                          │
     └──────────┬──────────┘                                          │
                │ the only sanctioned way to feed the model            │
                ▼                                                     ▼
         DescribeData (C6)                                    value_match lookup
         hardened schema + stats + k-anonymity          (small output × prebuilt index = fast)
                │                                                     │
         ┌──────┴──────── the two entry points into model context (both must be guarded) ────┴──────┐
         │  ① prompt injection ◄── ambient context ◄── only a safe profile is fed                    │
         │  ② tool results:                                                                          │
         │       on_tool_request ─►[ ingress pipeline: C2 blacklist → C4 review (exec) ]─► execute ─► output
         │       output ─► on_tool_result ─►[ egress pipeline: C3 row_cap → value_match → regex → C4 review ]─► fed back to the model
         │                per-strategy on_fail: pass|redact|block|ask; sequential execution + fail_fast │
         └───────────────────────────────────────────────────────────────────────────────────────────┘
         (C5 sandbox: exec no-network/read-only/separate user; C1 narrowing: optionally remove risky
          tools — both are enhancements, not the boundary)
