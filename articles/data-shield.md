...
**Language:** English \|
[简体中文](https://kaipingyang.github.io/codeagent/articles/data-shield-cn.md)
...
**three edges**. Data Shield installs a gate at each one:

1.  **User/model input** (edge 1, input gate) — typed text and
    text-bearing attachments are scanned before the model sees them.
    Framework-owned protected dataset context is separately generated as
    filtered schema-only metadata (names, types, dimensions, and
    policy-safe summaries), never raw rows.
2.  **Tool traffic** (edge 2, tool gate) — tool arguments are checked
    before execution and tool results are filtered before they are fed
    back to the model.
3.  **The model’s final reply** (edge 3, output gate) — the complete
    reply is scanned before it reaches the user, since the model may
    reproduce a protected value inferred from an edge-2 aggregate even
    when the user’s input was clean.

RAG content, attachments, and errors must enter through one of these
edges. The same policy applies recursively to owned foreground
sub-agents.

> **Image boundary:** image prompt attachments are not scanned by
> default. An opt-in OCR hook
> ([`data_shield_ocr_scanner()`](https://kaipingyang.github.io/codeagent/reference/data_shield_ocr_scanner.md))
> can scan text baked into those images at edge 1. Image/multimodal
> **tool results** still bypass text egress scanning and require a
> host-provided image-result control.
...
Every code block in this vignette continues from a `chat` (a bare
**ellmer** `Chat`, not the `CodeagentClient` wrapper) and a `shield` (a
`DataShield` instance) built once, up front:
...
`codeagent_client(chat, ...)` does **not** clone `chat` — `client$chat`
is the exact same object you passed in (mutable R6/environment
semantics), so a snippet may call either `chat$register_tool(...)` or
`client$chat$register_tool(...)` interchangeably; this vignette uses the
bare `chat` variable throughout for brevity.
...
The input and final-response gates are client settings rather than
`shield_*()` strategies. Their secure defaults are:
...
Unknown scanner names and scanner exceptions fail closed. Input `"ask"`
has no separate approval path and degrades to redaction. Text-bearing
attachments are scanned; if their text cannot be extracted, the turn is
blocked because an immutable attachment cannot be safely rewritten. When
a shield is active, CLI and Shiny streaming buffer the complete model
reply, scan it, and only then emit the safe text once—there is no
plaintext-first stream.
...
`tesseract` is an optional dependency. If it is absent, the ready-made
scanner degrades to pass (the image blind spot remains). Once OCR is
available and the scanner is configured, unreadable images, OCR errors,
and OCR-text scan errors block; detected protected text blocks by
default.
...
`category_max`, `category_ratio`, `audit_max`, `dp_epsilon`,
`dp_budget`) create the default DescribeData + core egress configuration
when `strategies = NULL`. `audit_max` defaults to 1000 non-sensitive
decision events; `0` disables recording. Supplying
`strategies = list(...)` enables **only** listed strategies. List order
controls the egress scanner pipeline; ingress, reviewer, sandbox, and
tool-policy strategies run at their distinct phases. Use
`shield$register_data()`, `$install()`, `$describe()`, `$audit()`,
`$clear_audit()`, `$clear()`, and `$close()` for dynamic/session-owned
workflows.
...
When protected data is registered **before**
[`codeagent_client()`](https://kaipingyang.github.io/codeagent/reference/codeagent_client.md)
is built, its filtered `DescribeData`-style schema is inserted into a
`<protected-data>` system-prompt block. This tells the model the real
dataset names, dimensions, and allowed columns without exposing
identifiers or rare categories. `DescribeData` remains the authoritative
live fallback.

`register_data()` updates the live engine and value index but
deliberately does not rewrite an existing Chat prompt. After a runtime
upload, refresh explicitly:
...
The refresh rebuilds the prompt while preserving history, tools,
budgets, hooks, and the same live `DataShield`; expect a one-time
prompt-cache miss. Under `distributions = "dp"`, both schema-block
generation and `DescribeData` consume the configured per-dataset privacy
budget.
...
- **`DescribeData`** is registered only when
...
- **`audit_code_tool(shield, project_root)`** is an opt-in `AuditCode`
  ...
  extensions inside `project_root`, never executes the submitted code,
  and returns risk/reference metadata rather than file contents. With a
  shield, whitelisted source text goes through the reviewer rail.
  `AuditCode` grants no read/write/shell capability to the main model.
...
Foreground sub-agents (`Agent`) inherit the exact same live `DataShield`
R6; their tools are installed behind the same central permission and
Shield gates before the first child request. This applies to synchronous
and concurrent async Agent calls. While a shield is active, raw
`btw_tool_agent_*`/custom delegation paths that cannot accept the engine
are not exposed. The child reply is output- gated before `SubagentStop`
hooks, callbacks, or the parent tool result, and shielded sidechains are
not persisted.
...
These reproduce the two “not safe alone” rows of the matrix above,
exactly as wired in `inst/examples/data_shield_minimal_app.R`’s “Shield
strength” selector — switch to one there, ask the chat to dump the
uploaded data, and watch it leak live.
...
- **`unsafe_ingress_only`** only scans tool *arguments* before execution
  (see
  [`shield_ingress()`](https://kaipingyang.github.io/codeagent/reference/shield_ingress.md)
  in the parameter reference above). It has no
  [`shield_egress()`](https://kaipingyang.github.io/codeagent/reference/shield_egress.md),
  so nothing inspects what a tool actually *returns*. Its static regex
  rules also only see the literal code text: `y <- study; print(y)` does
  not match any “print/dump a known dataset name” pattern, so the alias
  sails through — this is the exact bypass the combination-safety table
  above marks “confirmed”.
- **`unsafe_describe_only`** only registers the `DescribeData` tool (see
  [`shield_describe()`](https://kaipingyang.github.io/codeagent/reference/shield_describe.md)
  above). That tool is the model’s own sanctioned query path; it does
  nothing to any *other* tool’s return value, so a tool that simply
  returns the raw data.frame is never touched.
- Both omit
  [`shield_egress()`](https://kaipingyang.github.io/codeagent/reference/shield_egress.md)
  — see “Combination safety” above for why that is the one strategy
  every other combination depends on.
...
UNSAFE combos, so you can switch between all five in a running chat and
watch the exact leaks in the table happen live.
...
- **`value_match` scales linearly and is now bounded.** Benchmarked on
  ...
  caught. Because memory grows linearly,
  `register_data(max_index_values=)` caps the index (default 500 000,
  ~65 MB). If the cap would truncate the index, registration errors and
  refuses the partially indexed dataset; raise the cap, split the data,
  or explicitly use `NULL`/`Inf`. The `min_len`/`min_card` thresholds
  performed well on these ids and are unchanged.
- **Image tool results and full OS isolation remain roadmap** (see the
  status banner): a tool-returned rendered table/plot of raw rows
  bypasses text egress scanning, and the portable sandbox is a
  path/capability policy, not kernel isolation. Prompt-image OCR is a
  separate opt-in edge-1 control described above; its
  optional-dependency fallback is intentionally a documented blind spot.
- **`shinychat` text-bearing attachments ARE scanned at edge 1.**
