...
in the R process. The tool therefore advertises
`read_only_hint = FALSE`, `destructive_hint = TRUE`, and
`open_world_hint = TRUE`; these annotations are risk signals, not
enforcement. Use only with a trusted model and environment, add an
explicit permission override if required by the host, and use Data
Shield/ `DescribeData` rather than `ExploreData` for protected data. The
general `RunR` tool remains separately classified as execution-capable
and goes through its own permission decision.
