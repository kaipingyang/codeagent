...
The `display` field is produced with
[`shinychat::tool_result_display()`](https://posit-dev.github.io/shinychat/r/reference/tool_result_display.html)
and may use shinychat-specific HTML and framed-card behavior. A
third-party host should not parse that presentation HTML.
...
Common payloads include `list(text=)` for text/code, `list(df=)` for
tables, `list(images=list(list(mime=, b64=)), output=)` for images, and
`list(message=, detail=)` for errors. A host should still tolerate
...
Rich shinychat results use the official framed presentation; this does
not change the cross-UI artifact.
...
validates schema, shape, and supported version. Its default is
...
always extracts the portable text when possible.
...
Future artifacts are transported unchanged through adaptation and
...
This lets middleware forward a future artifact without destroying it
while an older UI reliably falls back to `tool_result_value(event)`. A
UI should opt in to a new version only after implementing that version’s
contract.

## Trust boundary and failure behavior

Artifact data and restored session metadata are untrusted input.
codeagent therefore follows these rules:

- malformed or unsupported artifacts fail softly to `value`;
- plain titles, text, Markdown, and character HTML are escaped before
  browser rendering;
- only explicit
...
the UI may enhance a valid artifact, but it must always have the
