...
本文件记录在
`codeagent`（v0.2.3）项目上完成的安全修复、后续复审及验证结果。
第一、二轮包含早期安全审计和一般代码审查记录；第三、四轮针对权限、路径、
委派、配置和外部内容等核心安全边界进行了系统性加固。

> 修复日期：2026-09-09 ~ 2026-09-14 修复范围：已报告的高危安全漏洞、PR
> \#1 review blocker 及后续独立复审发现
> 验证方式：多轮独立只读复审、源码定向回归、完整 package/browser
> 验证门槛 当前结论：PR \#1 的权限、plan、Bash 环境、Data
> Shield、ExploreData、prompt、 PreToolUse、model switch 与 Shiny
> rollback blocker 已全部修复；独立复审已明确给出
> `SECURITY_BLOCKERS_CLEARED` 和 `READY_FOR_FULL_REGRESSION`，完整
> testthat、 R CMD check、固定 tarball 安装、installed smoke、pkgdown
> 与真实 Chromium 双布局 验证均已通过。PR 在提交并推送验证过的 diff
> 前仍保持 Draft。
...
1.  [当前安全结论](#%E5%BD%93%E5%89%8D%E5%AE%89%E5%85%A8%E7%BB%93%E8%AE%BA)
2.  [第三轮：权限边界与预览加固](#%E7%AC%AC%E4%B8%89%E8%BD%AE%E6%9D%83%E9%99%90%E8%BE%B9%E7%95%8C%E4%B8%8E%E9%A2%84%E8%A7%88%E5%8A%A0%E5%9B%BA)
3.  [第四轮：配置来源、委派能力与网络内容边界](#%E7%AC%AC%E5%9B%9B%E8%BD%AE%E9%85%8D%E7%BD%AE%E6%9D%A5%E6%BA%90%E5%A7%94%E6%B4%BE%E8%83%BD%E5%8A%9B%E4%B8%8E%E7%BD%91%E7%BB%9C%E5%86%85%E5%AE%B9%E8%BE%B9%E7%95%8C)
4.  [第一轮：基于安全审计的修复](#%E7%AC%AC%E4%B8%80%E8%BD%AE%E5%9F%BA%E4%BA%8E%E5%AE%89%E5%85%A8%E5%AE%A1%E8%AE%A1%E7%9A%84%E4%BF%AE%E5%A4%8D)
5.  [第二轮：全面代码审查修复](#%E7%AC%AC%E4%BA%8C%E8%BD%AE%E5%85%A8%E9%9D%A2%E4%BB%A3%E7%A0%81%E5%AE%A1%E6%9F%A5%E4%BF%AE%E5%A4%8D)
6.  [第一、二轮已修复文件摘要](#%E7%AC%AC%E4%B8%80%E4%BA%8C%E8%BD%AE%E5%B7%B2%E4%BF%AE%E5%A4%8D%E6%96%87%E4%BB%B6%E6%91%98%E8%A6%81)
7.  [历史代码审查清单](#%E5%8E%86%E5%8F%B2%E4%BB%A3%E7%A0%81%E5%AE%A1%E6%9F%A5%E6%B8%85%E5%8D%95)
...
PR \#1 的正式 `CHANGES_REQUESTED` 复审确认原始六项 blocker
后，独立复审又发现 四项高影响集成问题；当前工作树已分别关闭：
...
上述回归与相关 Data Shield、server settings、prompt/model switch
测试均已通过。 最终独立安全与集成门禁明确输出
`SECURITY_BLOCKERS_CLEARED` 和
`READY_FOR_FULL_REGRESSION`。合并前全套验证结果如下：

- `devtools::test(reporter = "summary")`：3342 assertions、0
  failures；13 条为当前 ASCII locale 下已知的 Shiny UTF-8 source
  warning，4 项按设计因可选 tesseract/ Anthropic 测试凭据缺失而 skip。
- `devtools::check(error_on = "warning", cran = FALSE, document = FALSE)`：0
  errors、 0 warnings、4 NOTEs；NOTEs 为可选 tesseract、活动
  `.codegraph` socket、安装体积和 包内 worker `:::` 调用，R code
  problems 为 OK，check 内 testthat 为 OK。
...
- [`pkgdown::check_pkgdown()`](https://pkgdown.r-lib.org/reference/check_pkgdown.html)
  输出 `No problems found`。
- 真实 headless Chromium installed-package E2E：classic 的
  `core/pass/redact/block` 全部通过；page_chat 的 `core` 与 chat/panel
  width 断言通过。
...
用户要求停止在本地运行代码后，仅进行了源码静态复审和编辑，未启动
R、测试、 构建、安装、server 或
codegraph。以下问题已由当前实现交叉确认并完成修复，
但新增回归测试尚未运行：
...
新增测试覆盖审批回调传递、worker 后端快照与重建、RAG metadata、project
setup 拒绝、旧 memory 升级、Windows recovery copy 和 processx DNS
fallback。当前只能
确认静态接线完整；在用户允许恢复本地执行前，不能把新增测试标记为已通过。

关联路径也同步收口：worker
快照改为在全部非委派工具注册后获取；模型切换和 btw
动态工具组更新会重建委派闭包并保留审批回调；Data Shield 安装失败会中止
或回滚；计划模式始终允许 `ExitPlanMode`；session/memory 读取前执行
recovery sweep；全局 session lookup 可以在合法 projects root 内
fork；camelCase `effortLevel` 会规范化并传递给 worker。
...
| Shiny 文件边界 | lazy-load、选择、预览和附件入口统一校验 canonical workspace root；移除无边界 [`startsWith()`](https://rdrr.io/r/base/startsWith.html) 判断 |
...
BackgroundAgent 在 Data Shield 激活时保持禁用，使用可继承同一实例的前台
Agent 作为明确兜底。
...
- 权限 gate、路径规则、文件工具、文件预览、RunR、Team/Background/Agent、
  Data Shield 子 Agent、client factory 和 backend contract
  定向回归全部通过。
- Windows 当前环境不允许测试创建符号链接，因此 dangling-symlink
  用例按测试设计跳过； 其他 canonical-path 用例通过。
- 当前源码已通过 `pak::local_install(".", ask = FALSE, upgrade = FALSE)`
  安装为 本地 `codeagent` 0.2.3，并确认 `xml2` 运行时依赖可用。
- [`pkgdown::check_pkgdown()`](https://pkgdown.r-lib.org/reference/check_pkgdown.html)
  因当前环境没有 Pandoc 无法执行；未把环境缺失误报为检查通过。
- `codegraph sync` 因当前环境没有 `codegraph`
  可执行文件无法执行；代码与已安装包不受影响，
  符号索引需在安装该工具的环境中补同步。
...
不再使用固定核心工具集合，而是与独立 worker
共享同一配置快照和父级工具白名单。

后续独立复审追加并关闭了以下旁路：

- 项目设置改为正向允许列表；重复、空或无效顶层 JSON
  键会使整份项目配置被忽略， 防止重复键残留危险值。
- btw patch 使用 btw 自身的解析器检查每个 `path` 和 `move_to`；多目标
  allow 必须覆盖 全部目标，任一 deny 始终优先。
- 路径型工具在执行包装层接收与 gate 相同的 canonical 参数；无显式路径的
  `btw_tool_files_search` 也固定在 client cwd 执行。
- Worker
  快照除工具名外保存基础实现签名；同名异构工具不会因名称相同而被继承。
- 安全快照 cwd 不可被普通调用参数覆盖；仅 TeamCoordinate 内部成功创建的
  worktree 可使用显式 trusted override。
...
- 全部 R 源文件通过语法解析。
- settings、permissions、central gate、文件工具、worker/Agent、动态 btw
  group、 web citation、client factory 和 backend contract
  定向回归通过。
- 三次独立只读安全复审逐步发现并关闭重复 JSON 键、patch 多目标规则、 cwd
  绑定、动态旧快照、同名异构工具和 cwd 覆盖旁路；末次复审未发现
  CRITICAL/HIGH/MEDIUM 剩余问题。
- 全量测试仍存在本任务前已知的 addin helper、可选 shinyglass、code-audit
  和 custom agent fixture 失败；安全定向套件无失败。
...
| **修复** | 替换 [`system2()`](https://rdrr.io/r/base/system2.html) 为 [`processx::run()`](http://processx.r-lib.org/reference/run.md)（保持参数边界）；添加 `--` 参数分隔符防止模式被解析为选项；添加 `--no-config` 防止项目级 `.ripgreprc` 改变执行行为；在 `DESCRIPTION` 的 `Imports` 中添加 `processx` |

### V-03：Bash 只读命令前缀匹配绕过

| 项目 | 内容 |
|----|----|
| **漏洞** | [`check_permission()`](https://kaipingyang.github.io/codeagent/reference/check_permission.md) 使用 `.is_bash_readonly()`（`^ls\\b`、`^cat\\b` 等前缀正则）自动放行 Bash 命令；`ls; rm -rf /`、`find . -exec`、`env bash -c` 均可绕过 |
| **文件** | `R/permissions.R` |
| **修复** | 从 [`check_permission()`](https://kaipingyang.github.io/codeagent/reference/check_permission.md) 中移除整个 Bash 前缀自动批准逻辑；Bash 在 `default` 模式下始终需要确认 |

### V-04：团队 worker 权限继承

| 项目 | 内容 |
|----|----|
| **漏洞** | [`team_run()`](https://kaipingyang.github.io/codeagent/reference/team_run.md) 和 [`team_coordinate()`](https://kaipingyang.github.io/codeagent/reference/team_coordinate.md) 默认 `permission_mode = "bypass"`，worker 不继承父级策略 |
| **文件** | `R/team.R`, `R/team_board.R`, `R/async_agent.R` |
| **修复** | 默认权限从 `"bypass"` 改为 `"dont_ask"`；[`team_run()`](https://kaipingyang.github.io/codeagent/reference/team_run.md) 新增 `parent_rules` 和 `parent_policy` 参数，序列化传递给 worker；[`team_coordinate_tool()`](https://kaipingyang.github.io/codeagent/reference/team_coordinate_tool.md) 同理更新文档和默认值；[`.bg_spawn()`](https://kaipingyang.github.io/codeagent/reference/dot-bg_spawn.md) 默认改为 `"dont_ask"` |

### V-05：只读快捷路径跳过拒绝规则

| 项目 | 内容 |
|----|----|
| **漏洞** | `R/tools_gate.R` gate 函数中 `continue_after_shield` 对 `cap == "read"` 的工具提前返回 [`invisible()`](https://rdrr.io/r/base/invisible.html)，跳过 `settings$tools$overrides` 和 `check_permission` 中的显式 `deny` 规则 |
| **文件** | `R/tools_gate.R` |
...
| 项目 | 内容 |
|----|----|
| **漏洞** | `R/server_right.R` 将 [`commonmark::markdown_html()`](https://docs.ropensci.org/commonmark/reference/commonmark.html) 输出直接插入 Shiny 主页面，Malicious Markdown 文件可执行任意 JavaScript |
| **文件** | `R/server_right.R` |
...
| 项目 | 内容 |
|----|----|
| **漏洞** | [`.mcp_autoconnect()`](https://kaipingyang.github.io/codeagent/reference/dot-mcp_autoconnect.md) 从 settings.json 和项目 `.mcp.json` 自动读取 MCP 配置并启动子进程，但默认 `enable_all_project_mcp_servers = FALSE` 未生效 |
| **文件** | `R/mcp_client.R` |
| **修复** | 将 [`.mcp_autoconnect()`](https://kaipingyang.github.io/codeagent/reference/dot-mcp_autoconnect.md) 改为空操作（no-op），仅返回 `invisible(0L)`；仅显式 `codeagent_client(mcp_config = ...)` 可连接 MCP 服务器 |
...
| 修复项 | 文件 | 说明 |
|----|----|----|
| Wear 默认权限 | `R/wear.R` | [`wear_explore()`](https://kaipingyang.github.io/codeagent/reference/wear_explore.md) 默认 `permission_mode` 从 `"bypass"` 改为 `"default"` |
...
| Team 工具传递父级权限 | `R/query.R` | [`.register_all_tools()`](https://kaipingyang.github.io/codeagent/reference/dot-register_all_tools.md) 调用 [`register_team_tool()`](https://kaipingyang.github.io/codeagent/reference/register_team_tool.md) 时传入 `parent_rules` 和 `parent_policy` |
...
来源：对 `codeagent`（v0.2.3）全部约 80 个 R 源文件的静态审查，发现 54
个问题（10 HIGH / 25 MEDIUM / 19 LOW）。
...
| **问题** | `trimws(input$install_skill_pkg)` 直接传给 [`btw::btw_skill_install_package`](https://posit-dev.github.io/btw/reference/btw_skill_install_package.html)，包名中的 shell 元字符可能造成命令注入 |
...
21 个 MEDIUM 安全漏洞**。2026-09-11 已根据 `process@ecce470`
完成静态同步： 8 项已修复、5 项原判断已失效、1 项部分解决、7
项仍待处理。未运行 R、测试、
...
| M-08 | `R/compaction.R:263-300,406-548` | 待处理 | [`snip_old_tools()`](https://kaipingyang.github.io/codeagent/reference/snip_old_tools.md) 会先写入 Chat history，后续压缩阶段失败或提前返回时仍缺少统一 snapshot/rollback |
...
| \# | 文件 | 行号 | 问题描述 | 修复建议 |
|----|----|----|----|----|
| L-01 | `R/tools_fs.R` | 35 | `.gitignore` 等点文件的 [`tools::file_ext()`](https://rdrr.io/r/tools/fileutils.html) 返回 `"gitignore"`，标记为无效 markdown 语言标签 | 对点文件使用空扩展名 |
...
| L-16 | `R/tools_web.R` | 265-270 | HTML 实体解码器仅处理 6 个实体（如 `&amp;`），其余保留原始文本 | 使用 [`xml2::xml_text`](http://xml2.r-lib.org/reference/xml_text.md) 进行完整解引用 |
