...
The multi-tool "agent loop" is ellmer-internal (inside `chat$chat()`); codeagent
steers it entirely through the two registered callbacks (`on_tool_request` gate,
`on_tool_result` mid-loop compaction + hooks). The streaming path
(`codeagent_stream()` / REPL / Shiny) shares the same pipeline via
`.turn_setup()` / `.turn_teardown()` and the same callbacks.
