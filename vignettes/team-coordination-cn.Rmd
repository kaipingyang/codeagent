...
`team_run()` 和 `team_coordinate()` 需要 `mirai`；任务板操作需要 `DBI` 和 `RSQLite`。这两个底层 API 的工作进程权限模式仍默认为 `"bypass"`，因为守护进程无法响应交互式审批；只应对可信任务和环境使用。`team_lead()` 现默认使用 `"dont_ask"`，需要审批的操作会被拒绝，除非调用方显式选择其他模式。
